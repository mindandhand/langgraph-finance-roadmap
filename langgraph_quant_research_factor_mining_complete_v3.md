...
>
> **已完成前置模块**：`langgraph-demos/` 已完成 8 个独立可运行示例，覆盖 State / Node / Edge / Graph、LLM 路由、Tool Calling 循环、Checkpoint、Streaming、Human-in-the-loop、并行 Fan-out 和 Subgraph。本文不再重复教授这些 API 的入门用法，而是直接讲它们如何服务于严谨的量化研究。
>
> **版本说明（V3）**：基础 Demo 仓库进入维护模式。量化主线不再重复通用 API 教学；后续新增 LangGraph 实践以真实量化研究问题为载体，并必须包含明确的研究目标、执行语义和可测试验收项。依赖升级、兼容性修复或补齐关键运行语义时，仍可维护或增补基础 Demo。
...
## 4.1 已完成的 LangGraph 基础仓库

基础仓库保持独立，不要把它重新复制进量化项目：
...
这 8 个 demo 与量化研究路线的对应关系如下：

| 已完成 demo | 在量化研究中的直接应用 |
|---|---|
| 01 State / Node / Edge / Graph | 数据校验、因子计算、统计检验和报告节点的固定流水线 |
| 02 LLM Call 与分类路由 | 将研究请求路由到数据、因子、统计、组合或审查流程 |
| 03 Tool Calling 循环 | 在严格预算内补充预定义检查，不允许循环寻找正结果 |
| 04 Checkpointer | 保存研究线程、实验状态和待完成检查，支持中断恢复 |
| 05 Streaming | 流式展示数据门禁、回测、稳健性和审查进度 |
| 06 Human-in-the-loop | 在因子接受、解锁 Holdout 或进入纸面组合前暂停审批 |
| 07 Parallel Fan-out | 并行测试持有周期、股票池、市场阶段、成本假设和模型 |
| 08 Subgraphs | 封装数据审计、统计验证、组合评估及多 Agent 私有子图 |

基础 demo 已完成后，需要确认的不再是“会不会调用 API”，而是以下运行语义：

1. 并行节点更新同一字段时必须定义正确 Reducer。
2. 循环必须具有业务终止状态、递归上限和研究预算。
3. Checkpoint 只保存执行状态，不代替因子库和实验数据库。
4. `interrupt()` 前后的副作用必须满足幂等性要求。
5. Subgraph 应只接收最小必要上下文，并保留 Agent 私有状态。
6. 大型 DataFrame、回测明细和模型文件只能通过 Artifact Reference 传递。
7. LLM 路由和 Tool Calling 必须使用结构化输出与参数校验。
8. 多 Agent 的共识不能代替确定性复算和统计证据。

### 基础仓库完成标准

在进入量化主线前，应能不查教程解释并修改：

- State 的字段由谁读取、由谁更新；
- 固定边、条件边、循环边和动态 `Send` 的区别；
- `invoke()`、`stream()`、Checkpoint 和 `thread_id` 的关系；
- `interrupt()` 为什么必须搭配 Checkpointer；
- 子图共享 Schema 与私有 Schema 的差别；
- 循环、并行和恢复分别可能带来哪些重复执行风险。

基础仓库不再按 API 清单机械增加 Demo。除依赖升级、兼容性修复或补齐关键执行语义外，新增实践应直接进入量化研究场景。

### 后续 LangGraph 实践准入规则

一个新增练习只有同时满足以下条件，才进入量化主路线：

1. **绑定明确的量化问题**：例如点时数据门禁、因子验证、稳健性实验、成本情景、组合优化或独立复核。
2. **引入必要的执行语义**：说明为什么需要条件路由、循环、并行、Checkpoint、Interrupt、Subgraph 或多 Agent，而不是普通 Python 顺序代码。
3. **具有可测试验收项**：至少包含一个节点测试、Graph 集成测试、失败路径测试或幂等性测试。
4. **保持数值职责边界**：LLM 只负责解析、规划、审查和解释；数值计算必须由确定性 Python 完成。
5. **保留简化基线**：能够说明普通 Python Workflow 为什么不足，或证明 LangGraph 在恢复、路由、并行、审查方面提供了实际增量价值。

允许新增的练习示例：

| LangGraph 能力 | 必须绑定的量化任务 | 最低验收项 |
|---|---|---|
| Structured Output | 生成 `ResearchSpec` / `FactorSpec` | 非法字段、窗口和时间语义被拒绝 |
| Tool Calling Loop | 在预算内补齐预定义证据 | 达到终止状态或预算后必然停止 |
| Checkpoint | 长时间稳健性实验恢复 | 中断恢复后不重复写入实验产物 |
| Streaming | 展示数据门禁和实验进度 | 事件顺序与节点执行轨迹一致 |
| Interrupt / Resume | 解锁 Holdout 或接受候选因子前审批 | 未审批时不能进入下一阶段 |
| Dynamic `Send` | 并行测试周期、市场阶段和成本情景 | Reducer 正确合并且无结果覆盖 |
| Subgraph | 封装数据、统计、风险或组合模块 | 父子图 Schema 和私有状态边界清楚 |
| Multi-Agent | 假设生成、盲化复核和 Red-Team | 权限隔离、冲突记录和单 Agent 对照 |

不进入主路线的练习：

- 仅把普通函数改名为 Agent；
- 为展示某个 API 而虚构没有研究价值的流程；
- 让 LLM 计算 IC、回归、回测或优化结果；
- 没有终止条件、预算和失败路径的自由循环；
- 多 Agent 成本更高却没有独立复核或增量价值验证。

## 4.2 量化研究主仓库

量化研究主仓库建议使用 Monorepo：
...
Level 0 不是正式研究阶段，而是确保学习者具备进入路线所需的最低量化基础。LangGraph 通用入门已由独立的 8 个 Demo 完成，本级只做能力验收和量化语义迁移，不再重新教学基础 API。
...
- 能独立运行和修改 8 个基础 demo
- 能为 State 增加字段并追踪每个节点的更新来源
- 能给循环增加明确终止条件
- 能解释 Checkpoint、Streaming、Interrupt、Send 和 Subgraph 的适用边界
- 能识别并行更新时需要 Reducer 的字段
...
6. 将 `01-hello-graph` 改造成“加载数据 → 校验数据 → 输出摘要”的确定性 Graph。
7. 为一个循环 Graph 同时设置业务终止状态和最大迭代限制。
8. 解释为什么因子矩阵不能直接长期保存在 Graph State 中。

量化基础未通过时，先补充 Python、线性代数或概率知识；LangGraph 验收未通过时，只回看对应 demo，不新增通用框架课程。

---

# LangGraph—量化研究衔接里程碑

在正式进入 Level 1 前，完成一个不超过 300 行的桥接项目：
...
确定性数据校验
  ...
  └─ 通过 → 计算一个简单动量特征
                 ↓
             输出 Artifact Reference 和摘要
```

桥接项目必须满足：

- 使用 `TypedDict` 或 Pydantic 定义研究配置；
- 至少包含一个确定性条件路由；
- 所有数值计算均由普通 Python 函数完成；
- Graph State 不保存完整 DataFrame；
- 至少有一个 Node 单元测试和一个 Graph 集成测试；
- 能使用 `stream()` 展示执行步骤；
- 同一研究 ID 重复执行不会重复写入同一产物。

完成后，基础 Demo 仓库进入维护模式。此后的主线练习应优先绑定量化研究任务；只有在依赖升级、兼容性修复或发现关键运行语义缺口时，才单独增补通用 Demo。每个新练习必须写明量化目标、采用 LangGraph 的必要性和可测试验收项。
...
## 已有 LangGraph 能力在本级的应用

本级复用 `01-hello-graph`，不重新学习 StateGraph API。重点是建立正确的量化职责边界：

- 先用普通 Python 函数实现并测试统计计算，再包装为 Node。
- 线性 Graph 只负责编排“加载 → 计算 → 验证 → 报告”。
- Node 返回指标摘要和 Artifact Reference，不返回大型矩阵。
- LLM 不参与 OLS、标准误、Sharpe 或回撤等数值真值计算。
- 同一输入和版本重复运行时，确定性节点必须得到可复现结果。
...
本级复用 `01-hello-graph`、`02-llm-call` 和 `04-checkpointer-memory`：

- 使用确定性条件边实现数据门禁，而不是让 LLM 判断数据是否合格。
- 为每个数据节点定义输入字段、输出字段、失败类型和允许的副作用。
- 使用 Checkpoint 保存验证进度，但把数据版本和质量报告写入正式实验记录。
- State 只保存版本、状态、发现项和 Artifact Reference。
- 任一关键点时检查失败时，Graph 必须走拒绝路径，不能由 Agent 绕过。
...
## LangGraph 与 LLM 的量化应用

本级复用 `02-llm-call` 和 `03-tool-calling-agent`，新增的不是循环写法，而是研究约束：

- Message 和 Prompt Template 只用于表达研究意图。
- 使用 Structured Output 和 Pydantic Schema 生成 `FactorSpec`。
- Tool Calling 只能调用字段、操作符和窗口白名单。
- 工具参数必须经过类型、范围、时间语义和权限校验。
- Agent Loop 同时受到最大调用次数、研究预算和业务终止状态限制。
- 工具错误必须进入结构化回退路径，不能让模型猜测计算结果。
- 使用受约束因子 DSL，禁止模型自由生成并执行 Python。
- 确定性 Workflow 负责因子计算；Agent 只负责解析、选择和解释。
...
|---|---|---|---|
| 0 | Python、数学、金融前置 | 已完成 Demo 的能力验收 | 前置能力测试 |
| 1 | 概率统计、回归、研究工程 | 确定性节点、Artifact Reference、数值测试 | 量化统计工具库 |
| 2 | 点时数据、标签和数据质量 | 数据契约、门禁路由、失败终止 | 点时数据质量系统 |
| 3 | 因子假设、构造和预处理 | Structured Output、受约束 Tool Calling | 因子定义与计算 Agent |
| 4 | IC、分组、截面回归 | 固定研究 Workflow、盲化 Reviewer | 单因子研究系统 |
| 5 | 稳健性、多重检验 | 有界循环、研究预算、恢复与幂等 | 稳健性验证 Agent |
| 6 | 回测、成本和容量 | 动态并行情景、Reducer、失败隔离 | 成本约束回测系统 |
| 7 | 多因子、风险模型、优化 | 确定性 Subgraph Composition | 多因子组合系统 |
| 8 | 机器学习和时间验证 | 实验 Fan-out/Fan-in、统一评估 | ML Alpha 研究系统 |
| 9 | 因子发现、独立审查 | 多 Agent、Handoff、盲审、Interrupt | 多 Agent 因子研究系统 |
| 10 | 独立完整研究 | 综合编排、人工审批、证据追踪 | 半自动因子研究员 |
...
基础 LangGraph 能力已由独立的 8 个最小 demo 提供，因此本路线不再以 API 覆盖率为目标，也不要求建设通用平台。所有后续 Graph、Agent 和多 Agent 设计都必须证明其对量化研究质量、可复现性、审查或效率具有实际增量价值。
