...
下载器优先调用 AkShare 的 EastMoney ETF 日线接口，并按 `--adjust` 请求价格口径。如果 EastMoney 因网络或响应异常不可用，则使用精确交易所前缀标的（例如 `sh510050`）调用 Sina ETF 日线接口。Sina 不提供复权参数，因此回退数据按源端原样保存，不能保证符合请求的 `qfq`/`hfq` 口径；运行时会明确告警。
...
仓库当前内置的五标的快照由 EastMoney 主数据源生成，每个标的均含真实上游成交额，因此每个 feature 目录恰好有 7 个 `.day.bin` 文件。
